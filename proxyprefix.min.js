...
return true;},remove_param:function(el,options,uri){try{$.each(this.options.remove[uri.host()],function(key,value){if(uri.hasSearch(value)){uri.removeSearch(value);}});}catch(e if e instanceof TypeError){}
$(el).attr(options.attribute,uri.href());},append_param:function(el,options,uri){try{$.each(this.options.append[uri.host()],function(key,value){if(uri.hasSearch(key)===false){uri.addSearch(key,value);}});}catch(e if e instanceof TypeError){}
$(el).attr(options.attribute,uri.href());},extract_qurl:function(el,options,uri){return this.extract(el,options,uri,'qurl');},extract_url:function(el,options,uri){return this.extract(el,options,uri,'url');},extract:function(el,options,uri,param){var datamap=uri.search(true);var quri=URI(URI.decode(datamap[param]));console.log(quri._parts);if(uri.fragment()!==''){if(quri.path()!=="undefined"){quri.fragment(uri.fragment());}}
